...
shootings = read_csv("Mass Shootings Dataset.csv")

glimpse(shootings)
```
## shootings per year in different years
```{r}
  shootings %>%
  ...
  labs(x = 'Year', 
       y = 'Num of Shootings', 
       title = 'Num of mass shootings in the US') +
     coord_flip() +
     theme_bw() 
```
...
```{r}
raw_data<-read.csv('gun_regulation_raw_data.csv',sep=',',stringsAsFactors=F)
...
plotCategory<-function(x){
  ...
  #scale_x_discrete(name="",limits=c("1991","2001","2011","2017")) + 
  ylim(0,100) + facet_wrap(~Cat,ncol=2) + 
